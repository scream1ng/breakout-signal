...
def draw_interactive_chart(
    ...
    ticker:      str,
    stock:       dict,
    all_breaks:  list,         # from entry.detect_pivots — every break, full metadata
    ...
    # Signal markers with full analysis data
    signals_json = []
    for brk in all_breaks:
        ...
        # Colour scheme:
        #   Pink   (#ff6ec7) = all filters pass → same as PNG entry arrow (traded)
        #   Blue   (#2196F3) = high RVol (rvol >= rvol_min), RSM below threshold
        #   Green  (#26a69a) = normal/low RVol, RSM ok
        #   Yellow (#ffd740) = below RSM threshold
        #   Grey   (#888888) = below SMA50 (regime fail)
    ...
    data_json = json.dumps(dict(
        ...
        rsm_now=rsm_now, rsm_min=rsm_min, rvol_min=rvol_min,
