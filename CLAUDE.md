...
Open `http://localhost:8080` after starting app.
...
| `POST /api/trades/close` | Manually close position (paper mode only) |
...
2. Railway → **New** → **Deploy from GitHub repo** (auto-deploys on push to `main`)
...
**Automated schedule (APScheduler, runs inside web process):**
